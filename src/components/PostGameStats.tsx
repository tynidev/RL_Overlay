...
export class PostGameStats extends React.Component<PostGameProps, {}> {
  ...
  areTeamsEqual(
    ...
    for (let i = 0; i < oldLeft.length; i++) {
      if (!this.arePlayersEqual(oldLeft[i], newLeft[i])) return false;
    }

    for (let i = 0; i < oldRight.length; i++) {
      if (!this.arePlayersEqual(oldRight[i], newRight[i])) return false;
    }
  ...
  shouldComponentUpdate(nextProps: PostGameProps, nextState: {}) {
    return (
      ...
      this.props.series.teams[0].matches_won !==
        nextProps.series.teams[0].matches_won ||
      this.props.series.teams[1].matches_won !==
  ...
  render() {
    ...
    let [left_team, right_team, stats, mvp] = this.FillTeams(
      this.props.teams,
      this.props.left,
      this.props.right
    );
    ...
    let leftStatSliderWidth = {
      score:
        (360 - 10) * this.GetPercentage(stats.score.left, stats.score.right),
      goals:
        (360 - 10) * this.GetPercentage(stats.goals.left, stats.goals.right),
      assists:
        (360 - 10) *
        this.GetPercentage(stats.assists.left, stats.assists.right),
      shots:
        (360 - 10) * this.GetPercentage(stats.shots.left, stats.shots.right),
      saves:
        (360 - 10) * this.GetPercentage(stats.saves.left, stats.saves.right),
      demos:
        (360 - 10) * this.GetPercentage(stats.demos.left, stats.demos.right),
    };
    ...
    if (this.props.series.length > 0) {
      ...
      if (left_won > right_won) {
        if (left_won === games)
          series_txt =
            this.props.series.teams[0].name +
            ' wins ' +
            left_won +
            '-' +
            right_won;
        ...
        if (right_won === games)
          series_txt =
            this.props.series.teams[1].name +
            ' wins ' +
            right_won +
            '-' +
            left_won;
    ...
    return (
      <div
        ...
        style={{ opacity: this.props.display ? '1' : '0', transition: '400ms' }}
      >
